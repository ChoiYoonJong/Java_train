...
public class Run {
	...
	public static void main(String[] args) {
		...
		// add() : ArrayList데이터 추가
		// add 에 마우스를 올리면 Object 타입으로 들어가는것을 볼 수 있는데 이건은 컬렉션은 객체 타입만 받아주기 떄문에 기본자료형을 추가하면 내부적으로
		//AutoBoxing 에 의해 WrappeClass 로 변형되어 객체로 들어간다.
		a1.add(true);
		a1.add("이효리");
		a1.add('여');
		a1.add(20);
		...
		//remove(인덱스) : 원하는 인덱번째 값 삭제
		a1.remove(1);
		System.out.println("1번 인덱스 삭제후 :" + a1);
		
		//add() : 중간에 값 추가, 원하는 인덱스 번째
		a1.add(3, 37.85);
		System.out.println("3번 인덱스 삭제후 :" + a1);
		
		//set(인덱스, 변경할 값) : 원하는 인덱스 번째 값 수정
		a1.set(3, "abc");
		System.out.println("3번 인덱스 삭제후 :" + a1);
		
		//indexOf(찾을 값) :찾고자하는값의 인덱스 확인
		System.out.println(a1.indexOf("abcd"));
		
		//contains(찾을 값) : 찾을 값을 포함 했는지 (boolean)
		System.out.println(a1.contains('남'));
		
		//a1.clear(); //지닌 값을 모두지움
		a1.get(0); //저장된 크기가 0 인상태여서, IndexOutOfBoundsException
