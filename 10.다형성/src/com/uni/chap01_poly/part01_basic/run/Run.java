...
public class Run {
	...
	public static void main(String[] args) {
		...
		//1. 부모타입 레퍼런스 로 부모 객제 다루는 경우
		
		System.out.println("1. 부모타입 레퍼런스 로 부모 객제 다루는 경우");
		...
		//c1 레퍼런스로 Parent, child1 둘다 접근가능(Parent 멤버에 접근시 자동으로 형변화된체로 진행이 되고있었던것)
		...
		//하지만 Child1.에 접근을 해야한다면 형변화을 해야함 (명시적(강제) 형변환)
		p2.printParent();
		...
		//Child1 c2 = new Parent(); // 컴파일 에러 : 소스상의 문법 오류 (타입 맞지 않아서 에러 발생)
		//Child1 c2 =(Child1) new Parent(); //런타임 에러 발생: 프로그램이 실행시 발생하는 오류
		//c2.printParent();
		...
		//다형성이 적용된경우 - 하나의 부모타입의 레퍼런스로 다양한 자식 객체들을 다 받아줄 수 있다.(코드길이 감소, 유연한 코딩가능)
		...
		// 각각의 자식 메소드를 실행 -> 자식 타입으로 형변환을 해서 자식 메소드 실행 ((자식)부모).자식메소드()
		...
		for(int i = 0; i < arr.length ; i++) {
			...
			if(arr[i] instanceof Child1) {// 해당인덱스에 실제 Child1 타입을 참조하고 있으면 true 아니면 false
				((Child1)arr[i]).printChild1(); // 자식 1형변환
			}else {
				((Child2)arr[i]).printChild2(); // 자식 2형변환
			}
		...
		//for each문
		for(Parent p: arr) {
			if(p instanceof Child1) {// 해당인덱스에 실제 Child1 타입을 참조하고 있으면 true 아니면 false
				((Child1)p).printChild1(); // 자식 1형변환
			}else {
				((Child2)p).printChild2(); // 자식 2형변환
			}
		}
