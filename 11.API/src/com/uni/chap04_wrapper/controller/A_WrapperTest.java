...
public class A_WrapperTest {
	...
	public void method1() {
		...
		//1. 객체생성을 통해 하는방법
		Integer i1 = new Integer(num1); //생성자가 없어질꺼라서 줄로 그어져있게 나와있다.
		...
		System.out.println(i1.compareTo(i2));//두값을 비교해서 앞쪽이 크면 1변환, 뒤쪽이 크면 -1 반환, 같으면 0 반환
		
		//Integer i5 = "123";
		
		//2. 객체생성하지 않고 바로 대입하는 방법(AutoBoxing)
		// 하지만 객체 생성을 통해 Boxing 하게 되면 이렇게 문자열도 넣을 수 있지롱  --> 문자열을 숫자화 시켜줌 단, 문자열에 정수가 아닌 값이 있으면 오류!!
		Integer i3 = num2;
		...
		//1.Wrapper 클래스의 xxxValue() 를 통해 변호잔
		...
		int num5 = /*(int)*/i1; //구버젼 이후로 자동으로 된다.
