...
public class A_UnCheckedException {
	...
	public void method1() {
		...
		//int result = num1/num2; // 사용자가 num2에 0을 입력하면 비정상 종료
	...
	public void method2() {
		...
		System.out.println("배열의 길이 :");//음수로 확인
		int num = sc.nextInt();
	...
	public void classNarrayEx() {
		...
		try {
		//ClassCsetException : 잘못된 자료형 변환시 발생하는 예외 클레스
		...
		/*}catch(ClassCastException e) {
			System.out.println("잘못된 형변환을 하고있네! 다형성 복습하러가자");
		
		}catch(ArrayIndexOutOfBoundsException e) {
			System.out.println("배열크기를 잘못 지정했네~");
		}catch(NullPointerException e) {
			System.out.println("객체 없이 메소드를 쓰려하네");*/
		}catch(ClassCastException|ArrayIndexOutOfBoundsException|NullPointerException e) {
			//Multi catch 문 --> 나란히 작성하는 경우 형제 레밸만 함께 사용
			
			e.printStackTrace();// 에러발생 경로 + 에러메세지 (가장많이 자주사용한다.)
